################################################################################
# Doubly Robust Plug-in Estimator: HVICS Subscales -> DCS
#
...
#
# Exposure : HVICS subscales (4 items; each scored 4-36)
...
# Minimal sufficient adjustment set for HVICS models
...
covariates <- c(
  ...
  # Note: treated as a mediator in the DAG; included in the GPS model
  #       to improve propensity score estimation.
  "dm_style_actual",

  # Number of Decision Aids Used
  # Note: same mediator handling as dm_style_actual.
  "n_decision_aids"
)
...
calculate_weights <- function(df, exposure, covariates,
  ...
  poly_terms <- c(
    "I(age^2)",             "I(age^3)",
    "I(n_comorbidities^2)", "I(n_comorbidities^3)"
  )
...
#
# Outcome model uses linear covariate terms only (no polynomial terms
# for covariates in HVICS models).
################################################################################
...
bootstrap_dr <- function(df, exposures, covariates, n_iter = 1000) {
  ...
  for (exposure in exposures) {
    ...
    while (valid < n_iter && attempt < n_iter * 10) {
      ...
      fml <- as.formula(paste(
        "dcs_overall ~", paste(c("t_num", covariates), collapse = " + ")
      ))
